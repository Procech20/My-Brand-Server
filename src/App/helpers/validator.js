...
const Schema = {
	blogCreate: Joi.object().keys({
		title: Joi.string()
			...
			.min(10)
			.error(
				new Error('Please Add a title to your blog of atleast 10 characters!'),
			),
		content: Joi.string()
			.required()
			.min(250)
	...
	signup: Joi.object().keys({
		...
		firstName: nameSchema.error(
			new Error('firstname can only contain letters and numbers'),
		),
		surName: nameSchema.error(
			new Error('Surname can only contain letters and numbers'),
		),
		password: stringRequired
			.regex(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,}$/)
			.error(
				new Error(
					'password must be atleast 8 characters with atleast one lowercase letter, one uppercase letter and a special character',
				),
			),
		role: Joi.string(),
	}),
	login: Joi.object().keys({
		...
		password: stringRequired.error(new Error('password is required')),
	}),
};
